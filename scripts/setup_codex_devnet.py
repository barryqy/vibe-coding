...
def mcp_config_text() -> str:
    return "\n".join(
        [
            ...
            "",
            "[mcp_servers.mazemaker]",
            f"command = {toml_string(str(mcp_python()))}",
            f"args = [{toml_string('-m')}, {toml_string('dojo_app.maze_mcp_server')}]",
            f"cwd = {toml_string(str(ROOT))}",
            "",
        ]
    )
...
def ensure_mcp_config() -> None:
    ...
    if CONFIG.exists():
        ...
        text = text.replace(
            f"args = [{toml_string(str(ROOT / 'dojo_app' / 'maze_mcp_server.py'))}]",
            f"args = [{toml_string('-m')}, {toml_string('dojo_app.maze_mcp_server')}]",
        )
        text = ensure_mcp_cwd(text, "barryflights")
        text = ensure_mcp_cwd(text, "mazemaker")
        missing = []
        ...
        if "[mcp_servers.mazemaker]" not in text:
            missing.extend(
                [
                    "[mcp_servers.mazemaker]",
                    f"command = {toml_string(str(mcp_python()))}",
...
def main() -> int:
    ...
    if not base_url or not api_key:
        ...
        print("local_mcp=barryflights,mazemaker")
        return 0
