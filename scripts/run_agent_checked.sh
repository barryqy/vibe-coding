...
if grep -Eiq 'budget exceeded|HTTP 429|rate_limit' "${log_file}"; then
  scripts/cprint status "${status_key}=model-budget-exhausted"
...
if [ "${agent_rc}" -eq 124 ]; then
  scripts/cprint status "${status_key}=agent-timeout"
  exit 0
fi

if [ "${agent_rc}" -ne 0 ] || grep -Eiq 'ProviderModelNotFound|ModelNotFound' "${log_file}"; then
  scripts/cprint status "${status_key}=failed"
...
if grep -Eq "${expected_regex}" "${log_file}"; then
  scripts/cprint status "${status_key}=observed"
  exit 0
fi

scripts/cprint status "${status_key}=check-output"
