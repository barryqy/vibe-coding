...
- `bin/dojo-linux-x86_64` is a compiled learner artifact from the private challenge source repo. Do not replace it with source code or modify it in place.
- `config/dojo-event.toml` is the only event selector. The normal lab default is `self-paced`; event builds may replace it with a short event code.
- `scripts/player` is installed as `player` so learners can recall their assigned leaderboard name. Player names cannot be changed.
...
- `CLAUDE.md` is the optional Claude Code path for signed-in machines.
...
`LLM_MAZE_MODEL` overrides this task explicitly. Otherwise, use `LLM_HIGH_MODEL` before the normal `LLM_MODEL`. Keep `MAZE_MAX_ATTEMPTS` at its learner default of `1`; controlled evaluations may set it to `2` and use `MAZE_RETRY_MODEL` for one corrective attempt after the external verifier or compile check fails. Do not retry rate-limit, provider, budget, or timeout errors immediately.
...
- After OpenCode makes the Maze movement playable, compile the Maze files and then play the real interactive game:
