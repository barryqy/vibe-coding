...
- `CODEX_HOME=.lab-state/codex/home codex exec --disable plugin_sharing --ephemeral --cd "$PWD" --output-last-message .lab-state/codex-output/maze-plan.txt "Create a tiny Recursive Backtracker maze plan..."` to create the first maze plan
- `python3 -m dojo_app.maze_game --plan-file .lab-state/codex-output/maze-plan.txt --write-maze .lab-state/codex-output/maze.txt --check-only` to turn the plan into a valid, solvable maze artifact
...
Use Codex CLI with the DevNet model proxy for the required first result, the local BarryFlights MCP status-check mission, and the small Recursive Backtracker maze plan. Bring in OpenCode later to read the same second brain and make the generated Maze playable with a direct, small prompt. Keep Claude Code as an optional follow-up only when sign-in is already available.
